...
class HyDFSNode:
    ...
    def appendContent(self, file_content, HyDFSfilename):
        try:
            ...
            logging.error(f"Failed to append file to {HyDFSfilename}: {e}")

    def get_content(self, HyDFSfilename):
        flag = 0
        ...
        return self.read_folder_content(cache_path)
        
    ### helper func to append content to Hydfs file
    def appendFile(self, content, HyDFSfilename):
        self.appendContent(content, HyDFSfilename)
    
    ### helper func to return content from the Hydfs file   
    def getFile(self, HyDFSfilename):
        return self.get_content(HyDFSfilename)
    
    ### helper func to create a file in Hydfs
    def createFile(self, localfilename, HyDFSfilename):
        self.create_file(localfilename, HyDFSfilename)
        
    ### helper func to getAndMerge the file in Hydfs
    def getAndMerge(self, HyDFSfilename):
        self.merge_request_file(HyDFSfilename)
        time.sleep(2.5)
        return self.get_content(HyDFSfilename)
    
    def CheckGlobalFileList(self, hydfsfilename):
    ...
    def check_and_update_responsibilities(self):
        ...
        for filename in global_file_list:
            expected_responsible_nodes = self.get_responsible_nodes(filename)  
            # print("expect", expected_responsible_nodes)
    ...
    def get_responsible_nodes(self, filename):
        try:
            ...
            for i in range(self.replication_factor):
                ...
                print(f"Get_responsible_nodes: Error replicating: {e}") 
        finally:
            # print("type", type(i for i in responsible_nodes.keys()))
