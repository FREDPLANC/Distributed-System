...
import logging
# from hydfs_node import getvmnum
...
def getvmnum():
    ...
    return str(hostname.split('-')[-1].split('.')[0][3:])

class Node:
    def __init__(self, node_ip, node_port, introducer_ip=None,  detection_method = 'PingAck', drop_rate = 0, incarnation= None):
    ...
    def handle_udp_message(self, message, addr):
        ...
        if command == 'PING':
            ...
            if parts[4] == self.node_id:
                ...
                member_list, filename = self.decode_membership_list(parts[3])
                self.update_global_file_list(filename)
    ...
    def handle_command(self):
        while self.is_running:
            ...
            if command.strip() == 'enable_sus':
                ...
                # self.print_mem_list()
                aa = self.get_membership_list()
                bb = self.get_alive_machine()
                print(bb)
            elif command.strip() == 'list_file':
                self.logger.info(self.global_file_list)
        #TODO:
        pass
    ...
    def update_member_table_status(self, member_table):#***
        ...
        for node_id, member in member_table.items():
            ...
            if self_member:
                ...
                # print("faillist", self.faillist)
                if node_id in self.faillist:
                    continue
                else:
                    self.membership_list[node_id] = member
        self.lock.release()
    ...
    def update_member_status(self, node_id, status, timestamp): #***
        if status == 'Left':
            print("try to delete", node_id, status)
            self.lock.acquire()
            member = self.membership_list.get(node_id)
            if member:
                self.membership_list.pop(node_id, None)
                pass
            self.lock.release()
            return
    ...
    def mark_node_as_failed(self, target_node_id):
        ...
        if member and member['status'] != 'Failed':
            self.faillist.add(target_node_id)
            self.membership_list.pop(target_node_id, None)
        self.lock.release()
    ...
    def wait_for_ack(self, target_node_id, seq_num, time_out = 2):
        ...
        while time.time() - start_time < timeout:
            ...
            #         break
            with self.lock:
                if target_node_id not in self.membership_list:
                    return 
                
                if ((target_node_id, seq_num) not in self.pending_pings) or (
                    self.membership_list.get(target_node_id, {}).get('timestamp', 0) > int(start_time) and 
                    self.membership_list.get(target_node_id, {}).get('status') == 'Alive'
                ):
                    return
            # time.sleep(0.1)
    ...
    def encode_membership_list(self):
        ...
        with self.encode_lock:
            ...
            encoded_members = ';'.join(new_members)
            encoded_filenames = ','.join(self.global_file_list)
            return f"{encoded_members}|{encoded_filenames}"
    ...
    def decode_membership_list(self, data):
        membership_list = {}
        encoded_members, encoded_filenames = data.split('|')
        entries = encoded_members.split(';')
        for entry in entries:
        ...
        filename = encoded_filenames.split(',') if encoded_filenames else []
        filename = set(filename)
        return membership_list, filename
    def get_membership_list(self):
        ...
        aa = self.membership_list.copy()
        for ip_key, member in aa.items():
            node_id = self.get_vm_by_ip(hostlist, member['ip'].split(':')[0]) + '#' + str(member['ip'][-2:]) + str(member['incarnation'])[-3:]
            # member["node_id"] = node_id  # 
            if member['ip'] == '0000':
                continue
            new_membership_list[node_id] = member  # 

        if "0.0.0.0:0000" in new_membership_list:
            del new_membership_list["0.0.0.0:0000"]

        return new_membership_list
    def get_worker_membership_list(self):
        hostlist = [
            '172.22.157.76', '172.22.159.77', '172.22.95.76', 
            '172.22.157.77', '172.22.159.78', '172.22.95.77', 
            '172.22.157.78', '172.22.159.79', '172.22.95.78', 
        ...
        for ip_key, member in aa.items():
            node_id = self.get_vm_by_ip(hostlist, member['ip'].split(':')[0]) + '#' + str(member['ip'][-2:]) + str(member['incarnation'])[-3:]
            # member["node_id"] = node_id  # 
            if member['ip'] == '0000' or self.get_vm_by_ip(hostlist, member['ip'].split(':')[0]) == "vm1":
                continue
            new_membership_list[node_id] = member  # 
        if "0.0.0.0:0000" in new_membership_list:
            del new_membership_list["0.0.0.0:0000"]
    ...
    def get_alive_machine(self):
        ...
        for node_id, member in self.membership_list.items():
            if member['status'] == 'Alive' and self.get_vm_by_ip(hostlist, member['ip'].split(':')[0])!= "vm1":
                alive_machine.add(self.get_vm_by_ip(hostlist, member['ip'].split(':')[0]) + '#'+ str(member['ip'][-2:]) + str(member['incarnation'])[-3:])
        
        return alive_machine
    
    def get_global_file_list(self):
        return self.global_file_list
    def set_global_file_list(self, newlist):
        self.lock.acquire()
        self.global_file_list = newlist
        self.lock.release()
    
    # def update_global_file_list(self, new_files):
    #     self.global_file_list.update(new_files)
    def update_global_file_list(self, new_files):
        self.lock.acquire()
        if isinstance(new_files, set):
            self.global_file_list.update(new_files)
        else:
            self.global_file_list.add(new_files)  
        self.lock.release()
    ...
    def get_vm_by_ip(self, hostlist, ip):

        local_ip = self.get_local_ip(ip)
        if local_ip in hostlist:
            return f"vm{hostlist.index(local_ip) + 1}"  
        elif ip == '0000':
            return "vm0"
        else:
            return "unknown"
...
class Introducer(Node):
    def __init__(self, node_ip, node_port, detection_method='PingAck', drop_rate = 0, incarnation= None):
        dr = drop_rate
        super().__init__(node_ip, node_port, detection_method=detection_method, drop_rate=dr, incarnation= None)
        part = ["0000","0000"]
        self.membership_list["0.0.0.0:0000"] = {
                'ip': '0000',
                'port': '0000',
